...
- **2026-09-17 — USABLE-001 ordinary-terminal real-replay verification and lifecycle contract:**
  ...
  `ordinary_river_pending`, and **0** producing `ORDINARY_RIVER_CLOSED`. First blocker per hand:
  three of five named hands had no own-epoch credit confirmed strictly after their river frame
  (`payout_credit_not_visible`), one also had non-zero `pending_actions`, and the suspended
  `None` bucket has no epoch so it can never confirm. The credit channel is busy (218,310
  entries) but those are opening/refund credits, not post-river payouts. Second blocker located
  to the frame: `observed_deal_14593` DID satisfy C1+C2 (river at f15033, live pending across
  f15173-15185) but **f15186-15197 lost the complete-river reading**, resetting `river_frame`
  and withdrawing the pending record, and the epoch only switched at **f15198** (a direct A-to-B
  transition, no `None` in between), so C3 had nothing left. Minimal fix (clear `river_frame`
  only on an epoch change) is reported but was NOT applied, because this round's authorization
  was verification plus one lifecycle guard. The lifecycle contract was pinned instead:
