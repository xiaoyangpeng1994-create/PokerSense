...
## Current state
- **2026-09-16 STRATEGY-001-DIAG-V1 (Issue #23) CONCLUSION CORRECTION, NO KERNEL CHANGE:** Review 5217785775 (STRATEGY-001-R1) is correct: the production statistics path in tools/validate_threeway_models.py appends at most ONE record per (group, world) using an OR over three comparator columns, so the earlier claim that 9 of the 18 negatives were "double-counted" is wrong and has been withdrawn in docs/research. What remains true is only that the manual-reference column is numerically identical to check/call in the facing_bet worlds and to the selected policy in the unopened worlds - a readability/diagnostic observation that does NOT change the scenario failure count of 18. The earlier "main cause" and "over-calling" attributions are downgraded to untested hypotheses; no implementation error has been excluded without branch-level evidence. Added tests/tools/test_validate_threeway_models_counting.py covering the real production path: one negative comparator, two or three negative comparators in the same scenario, equal comparator columns, equal deltas across different (group, world) pairs, and an independent recomputation of the loss set from the report. Candidates, worlds, baselines, thresholds, the counting algorithm and the reported figure of 18 are all unchanged.
- **2026-09-16 STRATEGY-001 (Issue #23) OFFLINE RESEARCH, NO KERNEL CHANGE:** Reproduced the three-way river stress screen on baseline dc72a41 with no code change: validate_threeway_models.py exit 0 in 2s (30 world cases / 60 fixed-policy evaluations, fallback 0, FAIL_STRESS_SCREEN, NO_EMPIRICAL_PROMOTION); study_opponent_uncertainty.py exit 2 which is a by-design signal at tools/study_opponent_uncertainty.py:95, not a crash. Attribution: no category-A implementation defect found; the dominant cause is category-B response/range misspecification (value_heavy gives delta -80 chips versus check/fold). Two measurement facts newly quantified: in all 9 facing_bet rows the manual-reference comparator is exactly EV-equal to check/call, and in all 6 unopened rows it is exactly EV-equal to the selected policy, so 9 of the 18 negative comparisons double-count one comparison. Delivered docs/research/strategy-001-{evidence,findings,results}.zh-CN.md and kept the strategy kernel, candidate set, worlds and metrics unchanged. Deliberately REPORT_AND_REPRO_ONLY: no counter, no fallback-world relaxation, no new robust module, no profitability or live claim. Next batch task 1 = comparator-degeneracy detection in the report layer, with the original count preserved and an explicit falsification criterion.
