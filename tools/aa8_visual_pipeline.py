"""Offline AA8 development observations; unknown fields never enable advice."""
...
from tools.aa8_action_reader import AA8GlyphReader, GlyphTransitions, TEMPLATES
...
def run(source, target, bank_path, profile_path, output, context_source=None,
    ...
    tracker, events, counters = GlyphTransitions(), [], Counter()
    ...
    with (output / "observations.jsonl").open("w", encoding="utf-8") as stream:
        for frame, sample in targets.items():
            ...
            transitions = tracker.observe(frame, glyphs)
