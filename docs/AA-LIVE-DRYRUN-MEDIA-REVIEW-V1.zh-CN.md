...
操作者随后明确声明手机型号、Android 版本、AA App 版本和采集卡固件无法查询，
并声明本轮不要求识别视频转接器型号。这些字段以显式的操作者未知值进入声明，不能
被解释成已独立识别的设备事实。

新 session `aa-live-observer-dryrun-20260914-003` 使用完整声明指纹
`769a404c384d1105b2399c09750d194adf1d457824e8af606ee7d6fb993b7567`
完成了一次 10 秒旁观 dry-run。结果为 301 帧、输出时间 10.066656 秒、段终点
10.033 秒、0 丢帧、0 重复帧、退出码 0、非强制终止。完整收据检查退出码为 0，
结束后全局设备锁不存在，因而也真实验证了 PR #9 的锁释放修复。

新收据 SHA-256 为
`1259774ebc42c77204f506bfafb8515223c9ac24e3e7da328f5bb331806efdeb`。
本次没有解码或查看 session 003 的媒体内容。收据已经具备非空硬件声明指纹，但
`HARDWARE_DRY_RUN_SIGNED_OFF` 仍必须由采集结束后的人工签收产生；不能用采集前的
授权冒充事后签收。签收前不得创建或启动开发采集。
